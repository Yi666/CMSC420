...
public abstract class Graph {
    ...
    protected static final RuntimeException Negative_edge = new RuntimeException("Inacceptable Edge!");
    ...
    public List<Integer> shortestPath(int source, int dest) {
        ...
        int node_num = this.getNumNodes();

        int[] matrix_distance = new int[node_num];


        for (int i = 0; i < node_num; i++) {

            matrix_distance[i] = INFINITY;
        ...
        while (new_source != dest && this.getNeighbors(new_source) != null) {
            for (int i = 0; i < node_num; i++) {
                if (i != source && this.getNeighbors(source).contains(i)) {
                    if (matrix_distance[i] > matrix_distance[new_source] + this.getEdgeWeight(new_source, i)) {
                        matrix_distance[i] = matrix_distance[new_source] + this.getEdgeWeight(new_source, i);
                    }
                }
            }
        }
...
*/
        ...
        int node_num = this.getNumNodes();     //get the node number
        int minimum_distance = INFINITY;                //get the minimum distance
        int new_source = source;



        int[] distance = new int[node_num];
        boolean[] flag = new boolean[node_num];
        LinkedList[] path_route = new LinkedList[node_num];
        ...
        if (this.getNeighbors(source) != null) {
            for (int i=0;i<node_num;i++){
                if (i != source && this.getNeighbors(source).contains(i)) {
                    if (distance[i]>this.getEdgeWeight(source,i)) {
                        distance[i] = Math.min(distance[i], this.getEdgeWeight(source, i));
                    }
                }
            }
        }
        ...
        //from source to its neighbors, we found the shortest route from source
        ...
        for (int i=0;i<node_num;i++){
            if (distance[i] == minimum_distance){
                new_source = i;
                flag[new_source] = false;
                //System.out.println(new_source);
                path_route[new_source].add(new_source);
            }
        }

        while(new_source != dest){
            if (this.getNeighbors(new_source) != null) {
                for (int i=0;i<node_num;i++){
                    if (i != new_source && this.getNeighbors(new_source).contains(i)) {
                        if (distance[i] > (distance[source] + this.getEdgeWeight(source,i))) {
                            distance[i] = distance[source] + this.getEdgeWeight(source,i);
                            path_route[i].add(path_route);
                            path_route[i].add(i);
                        }else{
